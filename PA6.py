...
def database_insert(database,robot):
    nameHash = hash_function(robot.name)
    shapeHash = hash_function(robot.shape)
    colorHash = hash_function(robot.color)
    database.name_table[nameHash].append(robot)
    database.color_table[colorHash].append(robot)
    database.shape_table[shapeHash].append(robot)
...
def database_search(database,field,value):
    ...
    out = []
    if field == "shape":
        for i in database.shape_table[hash]:
            if i.shape == value:
                out.append(i)
    elif field == "color":
        for i in database.color_table[hash]:
            if i.color == value:
                out.append(i)
    elif field == "name":
        for i in database.name_table[hash]:
            if i.name == value:
                out.append(i)
    return out
#Takes Database object database, string field (must be "name","shape","color")
...
def database_delete(database,field,value):
    ...
    if field == "name":
        for i in database.name_table[hash]:
            if i.name != value:
                newList.append(i)
        database.name_table[hash] = newList
        newList = []
        i = 0
        while i < len(database.shape_table):
            for j in database.shape_table[i]:
                if j.name != value:
                    newList.append(j)
            database.shape_table[i] = newList
            newList = []
            i += 1
        i = 0
        while i < len(database.color_table):
            for j in database.color_table[i]:
                if j.name != value:
                    newList.append(j)
            database.color_table[i] = newList
            newList = []
            i += 1
        i = 0
    elif field == "shape":
        for i in database.shape_table[hash]:
            if i.shape != value:
                newList.append(i)
        database.shape_table[hash] = newList
        newList = []
        i = 0
        while i < len(database.name_table):
            for j in database.name_table[i]:
                if j.shape != value:
                    newList.append(j)
            database.name_table[i] = newList
            newList = []
            i += 1
        i = 0
        while i < len(database.color_table):
        ...
        for i in database.color_table[hash]:
            if i.color != value:
                newList.append(i)
        database.color_table[hash] = newList
        newList = []
        i = 0
        while i < len(database.name_table):
            for j in database.name_table[i]:
