...
def max_heapify(A,i):
    L = Left(i)
    R = Right(i)
    if L < len(A) and A[L].priority > A[i].priority:
        largest = L
    else:
        largest = i
    if R < len(A) and A[R].priority > A[largest].priority:
        largest = R
    if largest != i:
...
def insert_task(task_heap,task):
    ...
    increase_task_priority(task_heap, int(len(task_heap) - 1), task.priority)
...
def extract_max_priority_task(task_heap):
    if len(task_heap) == 0:
        return None
    task_heap[0], task_heap[-1] = task_heap[-1], task_heap[0]
    out = task_heap.pop(-1)
...
def increase_task_priority(task_heap,i,new_priority):
    ...
    while i > 0 and task_heap[i].priority > task_heap[parent(i)].priority:
