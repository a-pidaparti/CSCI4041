...
def point_insert_point(P, newPoint, depth):

    dim = depth % 2   #calculates which dimension to split based off of
    if newPoint.coords[dim] > P.coords[dim]:
        if P.right == None:
            P.right = newPoint
            newPoint.parent = P
        else:
            point_insert_point(P.right, newPoint, depth + 1)
    else:
        if P.left == None:
            P.left = newPoint
            newPoint.parent = P
        else:
            point_insert_point(P.left, newPoint, depth + 1)
...
def insert_point(kd_tree,point):
    if kd_tree.root == None:
        ...
        point_insert_point(kd_tree.root, point, 0)
...
def point_nearest_neighbor(P, point, best, depth):
    if P == None:
        return None
    curBest = closer(point,P,best)          #if current node is closer, update best
    dim = depth % 2
    if point.coords[dim] > P.coords[dim]:   #compare 1 coordinate and assign near and far
        near = P.right
        far = P.left
    else:
        near = P.left
        far = P.right
    possibleBest = point_nearest_neighbor(near,point,curBest,depth+1)   #recursively search near branch
    if possibleBest != None:
        curBest = closer(point, possibleBest, curBest)
    radius = point.dist(curBest)

    if (abs(P.coords[dim] - point.coords[dim])) < radius:       #check if we need to care about far branch
        possibleBest = point_nearest_neighbor(far,point,curBest,depth + 1)
        if possibleBest != None:
            curBest = closer(point, possibleBest, curBest)

    return curBest  #return best

def nearest_neighbor(kd_tree,point):
    if kd_tree.root == None or point == None:
        return None
    return point_nearest_neighbor(kd_tree.root, point, kd_tree.root, 0)
