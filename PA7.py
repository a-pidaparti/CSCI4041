...
def real_insert_floor(floor, treeNode):
    if treeNode.right == None and floor.floor_num > treeNode.floor_num:
        treeNode.right = floor
    elif treeNode.left == None and floor.floor_num < treeNode.floor_num:
        treeNode.left = floor
    elif floor.floor_num > treeNode.floor_num:
        real_insert_floor(floor,treeNode.right)
    elif floor.floor_num < treeNode.floor_num:
        real_insert_floor(floor, treeNode.left)
...
def insert_floor(floor_tree,floor):
    if floor_tree.root == None:
        floor_tree.root = floor
        return

    real_insert_floor(floor, floor_tree.root)
...
def delete_floor(floor_tree,floor):
    if floor.left == None:
        ...
        succ = successor(floor)
        if succ.parent != floor:
            transplant(floor_tree, succ, succ.right)
            succ.right = floor.right
            succ.right.parent = succ
        transplant(floor_tree, floor, succ)
        succ.left = floor.left
        succ.left.parent = succ

def real_find_nearest_unhaunted(treeNode, floor_num):
    print
    if treeNode.floor_num == floor_num:
        return treeNode
    elif treeNode.left != None and floor_num < treeNode.floor_num:
        return real_find_nearest_unhaunted(treeNode.left, floor_num)
    elif treeNode.right != None and floor_num > treeNode.floor_num:
        return real_find_nearest_unhaunted(treeNode.right, floor_num)
    elif treeNode.parent == None:
        return
    elif treeNode.parent != None:
        if abs(treeNode.parent.floor_num - floor_num) < abs(treeNode.floor_num - floor_num):
...
def find_nearest_unhaunted(floor_tree,floor_num):
    ...
    return real_find_nearest_unhaunted(floor_tree.root, floor_num)
